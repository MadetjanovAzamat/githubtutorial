...
function searchMealData(mealName) {
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${mealName}`)
    .then(res => res.json())
    .then(data => renderMealData(data))
    .catch(() => {
        mealsContent.innerHTML = `<h1 class="bg-slate-300 text-center text-[40px] font-semibold my-[30px] text-slate-600">Food is not found!</h1>`
    })
}
searchMealData('Beef')

function renderMealData(data) {
    mealsContent.innerHTML = ''
    data.meals.map((item) => {
        mealsContent.innerHTML += `
        <div class="shadow-md rounded-md p-4">
